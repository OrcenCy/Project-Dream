...
func _ready() -> void:
	...
	for n in player_characters:
		player_characters[n].current_health
...
func turn() -> void:
	...
	print("Curent turn is: ", turn_order[0].char_stats.character_name)
	var turn_end = false
	if turn_order[0].char_stats.player:
		#This is a player controlled character
		#player can move and use an action or they can end their turn early
		#player chooses to make turn_end true
		if turn_order[0].current_health <= 0:
